...
# CHECK ----
	...
	output$df_check_qctable1_DT <- DT::renderDT({
		...
		# QC_Passed
		df_table <- df_table |>
			dplyr::mutate(QC_ExpectedColumns = grepl("No missing columns", ExpectedColumns),
							  QC_ExpectedDatatypes = grepl("All expected datatypes confirmed|all columns were generated by code", ExpectedDatatypes)) |>
			dplyr::mutate(QC_Passed = as.logical(QC_ExpectedColumns * QC_ExpectedDatatypes))
		...
		DT::datatable(df_table,
			...
			DT::formatStyle("QC_Passed",
								 ...
								 backgroundColor = DT::styleEqual(
								 	...
								 	c(color_good, color_bad))) |>
			# bold cells
			DT::formatStyle("QC_ExpectedColumns",
								 target = "cell",
								 fontWeight = DT::styleEqual(
								 	c(TRUE, FALSE),
								 	c("normal", "bold"))) |>
			DT::formatStyle("QC_ExpectedDatatypes",
								 target = "cell",
								 fontWeight = DT::styleEqual(
								 	c(TRUE, FALSE),
								 	c("normal", "bold"))) |>
...
# CAND CAUSE ----
	...
	names_label <- reactive({
		
		req(react_report_run())
		
		path_chem_names <- file.path(dn_results, 
											  react_setup_region(),
											  "_CheckedInputs",
											  "data_chemInfo.rds")
		
		if(file.exists(path_chem_names)){
			chem_names <- readRDS(path_chem_names) |> 
		...
		path_model_names <- file.path(dn_results, 
												react_setup_region(),
												"_CheckedInputs",
												"data_modelInfo.rds")
		
		if(file.exists(path_model_names)){
			model_names <- readRDS(path_model_names) |> 
				dplyr::select(StdParamName, Label)
		} else{
			model_names <- NULL
		}
		
		names_ret <- chem_names |> 
			dplyr::bind_rows(model_names)
...
# STRESS SUMM ----
	...
	# })
	output$stresssum_frame <- renderUI({
		req(ss_html_content())
		tags$iframe(
			srcdoc = ss_html_content(),              # your reactive HTML string
			style  = "width:100%; height:80vh; border:0;",
			# sandbox = "allow-same-origin allow-scripts"  # optional, if you need isolation
		)
	})
