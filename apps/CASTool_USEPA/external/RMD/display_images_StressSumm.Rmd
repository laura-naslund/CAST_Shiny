...
# Set the image directory — adjust if needed
...
for (i in seq_len(length(bc_dir))) {
	...
	png_all <- data.frame(files = list.files(
	  path = img_dir,
	  pattern = "\\.png$",
	  full.names = TRUE,
	  recursive = TRUE,
	...
	fp_elimStress <- file.path(dn_results,
								react_setup_region(),
								sel_targsite(),
								paste0(sel_targsite(), "_StressorsEliminated.csv"))
	
	elimStress <- read.csv(fp_elimStress) |> 
		dplyr::filter(Biocomm == tolower(bc_dir[i])) |> 
		dplyr::pull(Stressor)
	
	fp_responses <- file.path(dn_results,
								react_setup_region(),
								sel_targsite(),
								paste0(sel_targsite(), "_Responses.csv"))
	
	responses <- read.csv(fp_responses) |> 
		dplyr::filter(BioComm == tolower(bc_dir[i])) |> 
		dplyr::filter(IndexYN == "Y" | SSI == "Y") |> 
		dplyr::pull(MetricName)
	
	# Exclude files with eliminated stressors and WoE files
	excl_str <- paste0("/", elimStress, "/") |> paste(collapse = "|")
	
	incl_resp <- paste(responses, collapse = "|")
	
	png_incl <- png_all |> 
		dplyr::filter(!stringr::str_detect(files, excl_str))  |> 
		dplyr::filter(!stringr::str_detect(files, "_WoE")) |> 
	 	dplyr::filter(stringr::str_detect(files, incl_resp)) 
	
	png_incl_vec <- png_incl |> 
		dplyr::pull(files)
	
	# Exclude files with non-index responses
	# png_incl |> 
	# 	dplyr::filter(stringr::str_detect(files, "_CO.png"))
	# 
	...
	png_files <- sort(normalizePath(png_incl_vec))
	## directory names only
	png_dirs <- basename(dirname(png_files))
	## convert to labels
	png_dirs_labs <-  list.Stress$data_stressInfo[
		match(png_dirs, list.Stress$data_stressInfo$StdParamName),
		"Label",
		TRUE] # convert to vector from tibble
